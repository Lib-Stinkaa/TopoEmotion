...
class SimpleMLP(nn.Module):
    def __init__(self, input_dim=4144, num_classes=2):
...
def loso_cross_validation(df, feature_cols, target='valence'):
    ...
    for test_subject in subjects:
        ...
        model = SimpleMLP(input_dim=len(feature_cols), num_classes=2).to(DEVICE)
...
def main(features_file):
    ...
    print(f"MLP - Valence: Acc={valence['accuracy_mean']:.4f}±{valence['accuracy_std']:.4f}, "
          f"F1={valence['f1_mean']:.4f}±{valence['f1_std']:.4f}")
    print(f"MLP - Arousal: Acc={arousal['accuracy_mean']:.4f}±{arousal['accuracy_std']:.4f}, "
